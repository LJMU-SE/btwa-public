...
function NavLink({ href, onClick, children }: NavLinkProps) {
    return (
        <Link href={href} onClick={onClick} className="py-2 px-5 block md:inline-block text-white hover:text-orange transition-all">
            {children}
        </Link>
    );
}
    ...
    return (
        <div>
            <button onClick={toggleOpen} className={"absolute md:hidden text-black z-50 top-0 left-0 flex mx-5 items-center w-16 h-16"}>
                <FiMenu color={open ? "white" : "black"} size={20} />
            </button>
            <nav className={`${open ? "w-full" : "w-0"} fixed top-0 md:block overflow-hidden transition-all h-[100dvh] md:h-16 md:w-full bg-black`}>
                <ul className="mt-16 md:mt-0 h-full flex flex-col md:flex-row md:justify-between md:items-center">
                    <Link href="/">
